...
func (user User) EnableToPayment() bool {
	if user.CPF != "" && !IsCpfValid(user.CPF) {
		return false
	} else if user.CNPJ != "" && !IsCnpjValid(user.CNPJ) {
		return false
		...
		return false
	} else if user.AddressState == "" {
		return false
	} else if user.AddressNeighborhood == "" {
		return false
...
func (user User) GetUserTypeString() string {
	switch user.Type {
	case USER_TYPE_ADMIN:
		return "Administrador"
	case USER_TYPE_MANAGER:
		return "Gerente"
	case USER_TYPE_CONTROLLER:
		return "Controlador"
	case USER_TYPE_STOCKMAN:
		return "Estoquista"
	case USER_TYPE_DELIVERY:
		return "Entregador"
	default:
		return "Usuário"
	}
...
func (user User) IsCollaborator() bool {
	switch user.Type {
	case USER_TYPE_CONTROLLER:
		return true
	case USER_TYPE_STOCKMAN:
